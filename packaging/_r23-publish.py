...
COMMIT_MSG = """第 25 轮：免安装版实测反馈修复（9 项）

- 漫画库 PDF 阅读器可正常打开与翻页
- 小说库 EPUB 正文插图可加载；新增「滚动 / 翻页」双阅读模式
- 我的收藏新增展示设置（类型筛选 / 排序 / 每页件数 / 摆放位置）
- 演员头像记录回填；标签库打开即为铺满网格；年度报告圆形不再闪烁
- 海报健康：候选按内容类型分流（视频截帧 / PDF 页 / EPUB 内封面）+
  空态诊断 + 手动上传兜底始终可达
- AI 设置「获取」可刷新出可用模型列表
- 修复设置页自「AI 设置」起五组内容错位到下方的问题
- 侧栏四个库的计数首屏即可显示
- 发布脚本增加网络重试（宿主代理偶发 SSL EOF / 502）
"""
