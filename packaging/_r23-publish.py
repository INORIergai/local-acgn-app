...
COMMIT_MSG = """重制公开版本：脱敏 + 桌面版 exe 支持

- 移除个人配置（config.json / config.docker.json）与全部调试脚本
- 个人目录结构、内网 IP 统一替换为通用占位
- 访问密码改为可选开关（默认关闭），修改/关闭需验证当前密码
- 修复首次启动后端因 ESM-only 依赖无法 require 而静默退出的问题
- 新增 Electron 桌面版打包链（安装版 / 便携版）
"""
...
def api(method, path, body=None, timeout=90):
    ...
    try:
        ...
        return 0, '%s: %s' % (type(e).__name__, str(e)[:220])
