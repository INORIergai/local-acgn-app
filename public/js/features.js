...
(function () {
    ...
    async function openWatchTab(key, searchQ) {
        const site = WATCH_SITES.find((s) => s.key === key);
        if (!site) return;
        const open = watchTabs.find((t) => t.key === key);
        if (open) { activateWatchTab(key); if (searchQ) navigateWatchTab(open, searchQ); return; }   // 已开过就切过去并补一次搜索
        ...
        const tab = { key, name: site.name, pane, board: site.board || watchBoard, initialQuery: searchQ || '' };
        watchTabs.push(tab);
        freshTabs.add(key);
    ...
    async function loadWatchPane(tab, site, force) {
        ...
        if (info.frameable === false) {
            if (info.canList) return fillPaneList(pane, site, tab, q0);
    ...
    // ================= 导出 =================
    // round34 批次B：榜单「去观看/去阅读」联动入口。
    // viewName: 'watch-anime' | 'watch-comic' | 'watch' | 'watch-hanime'；q: 搜索词。
    window.openWatchWithSearch = async function (viewName, q) {
        if (!q) return;
        watchBoard = BOARD_OF_VIEW[viewName] || 'av';
        // 切到对应视图（switchView 由 app.js 全局提供）
        if (typeof window.switchView === 'function') window.switchView(viewName);
        await showWatch(viewName);
        const pref = (WATCH_SITES.filter((s) => (s.board || 'av') === watchBoard && (SITE_CAPS[s.key] || {}).canSearch)
            .map((s) => (SITE_CAPS[s.key] || {}).canProxy ? { ...s, _w: 0 } : { ...s, _w: 1 })
            .sort((a, b) => a._w - b._w)[0]) || WATCH_SITES.find((s) => (s.board || 'av') === watchBoard);
        if (pref) await openWatchTab(pref.key, String(q).trim());
    };
