...
async function hardRefreshCurrent() {
    ...
    try {
        const special = ['settings', 'annual', 'new-releases', 'scrape-failures',
            'watch', 'watch-anime', 'watch-comic', 'watch-hanime',
            'guess', 'guess-magic', 'playlists'];
            ...
            <main class="settings-main">
                ...
                <section class="settings-section" data-group="version" style="display:none;">
                    ...
                    <div class="settings-group" id="updNewBox" style="display:none;">
                        ...
                        <div class="settings-btn-row">
                            <button class="btn btn-sm" style="background:var(--primary);" onclick="downloadAppUpdate()">⬇️ 下载更新</button>
                            <button class="btn btn-sm" onclick="cancelUpdateDownload()">✖️ 取消下载</button>
                        </div>
                        <div id="updProgressWrap" style="display:none;margin-top:10px;">
                            ...
                            <div id="updProgressText" style="font-size:12px;color:var(--text-muted);margin-top:6px;"></div>
                        </div>
                        <div class="settings-btn-row" style="margin-top:10px;">
                            <button class="btn btn-sm" onclick="installAppUpdate()">🚀 立即安装</button>
                            <button class="btn btn-sm" onclick="openUpdateFolder()">📂 打开下载目录</button>
                        </div>
...
function renderPosterOptions(grid, movieId, scraped, frames, cfg, state) {
    ...
    // —— 1. 刮削结果 ——
    if (!scraped) {
        html += `<div class="poster-section-label" style="opacity:.7;">🔍 刮削结果</div>`
    ...
    if (!frames) {
        html += `<div class="poster-section-label" style="opacity:.7;">🎞️ 内容截图</div>`
    ...
    html += `
        <div style="grid-column:1/-1;margin-top:12px;display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;">
            ...
            <span style="display:flex;gap:8px;flex-wrap:wrap;">
                <button class="btn btn-sm btn-secondary" onclick="showChangePosterModal(${movieId})">🔄 重新刮削</button>
        ...
        <div class="detail-scroll">
            ...
            <div class="detail-section">
                ...
                <div class="detail-actions">
                    ...
                    <button class="dbtn" title="编辑标题与元数据" aria-label="编辑元数据" onclick="showEditModal(${movie.id})">📝</button>
...
async function refreshWatchStatus() {
    try {
        ...
        if (!statusEl) return;
        if (!st.enabled) {
            statusEl.textContent = '目录监控已关闭';
            statusEl.title = '开启后：应用会持续盯住四个媒体目录，新文件自动入库并刮削';
            return;
        }
        const parts = [];
...
function init() {
    ...
    /* ★ round26 #1：优先恢复 hash 里记着的视图与搜索词。
     *   原来 F5 / 手动刷新后一律回到「全部影片」，用户搜索到一半换完海报再刷新，
     *   就再也回不到原来的结果页。现在视图与搜索词都写进 hash，刷新原地恢复。 */
    ...
    } else if (typeParam && ['jav', 'anime', 'comic', 'novel'].includes(typeParam)) {
