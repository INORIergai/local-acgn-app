...
async function loadAIModels(provider) {
    try {
        ...
        if (data.code === 0 && data.data?.models) {
            ...
            if (statusEl) statusEl.textContent = '❌ 加载失败: ' + (data.data?.error || data.msg || '未知错误');
        }
    } catch (e) {
        const statusEl = document.getElementById(provider + 'TestStatus') || document.getElementById('aiConfigStatus');
...
// 显示更换海报弹窗（刮削结果 + 视频截帧候选，都没刮到时给上传入口）
async function showChangePosterModal(movieId) {
    currentMovieId = movieId;
    const modal = document.getElementById('posterModal');
    const grid = document.getElementById('posterGrid');

    grid.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-muted);">正在搜索海报…（同时从视频里抽帧）</div>';
    modal.classList.add('show');

    const [scraped, frames] = await Promise.all([
        fetch(`/api/movie/${movieId}/search-posters`).then(r => r.json()).catch(() => ({ code: -1 })),
        fetch(`/api/movie/${movieId}/frame-candidates`).then(r => r.json()).catch(() => ({ code: -1 }))
    ]);
    ...
    const card = (p) => `
        <div class="poster-option" onclick="changePoster(${movieId}, '${p.url}')">
            ...
            <div class="poster-option-info">
                <span class="poster-option-source">${p.source.toUpperCase()}</span>
            </div>
        </div>`;
    ...
    if (found.length) {
        ...
        html += `
            <div style="grid-column:1/-1;text-align:center;padding:12px 8px;color:var(--text-muted);">
                <div>未刮削到任何封面${shots.length ? '，可以从下面的视频截图里挑一张' : ''}</div>
    ...
    if (shots.length) {
        html += `<div style="grid-column:1/-1;margin:14px 0 2px;font-size:13px;color:var(--text-muted);">视频截帧（点一张就用作封面）</div>`
            + shots.map(card).join('');
    }
    grid.innerHTML = html;
}
...
async function loadFavorites(page = 1) {
    currentPage = page;
    const movies = await api.getMovies(currentSort, 'favorite', 'all', page, currentPageSize);
    renderMuseum(movies);
    renderPagination();
    updateToolbarTitle('我的收藏', currentTotal);
}
