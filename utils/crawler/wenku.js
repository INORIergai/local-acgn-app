...
class WenkuCrawler {
  constructor(config = {}) {
    this.baseUrl = config.baseUrl || 'https://www.wenku8.net';
  ...
  async search(keyword) {
    try {
      ...
      // 解析搜索结果列表
      $('.novel-item, .book-item, .item, .list-item, .result-item').each((i, el) => {
        const $el = $(el);
      ...
      console.log('[轻小说文库] 搜索失败:', e.message);
      return [];
    }
  }
  ...
  async getDetail(url) {
    try {
      ...
      console.log('[轻小说文库] 获取详情失败:', e.message);
      return null;
    }
  }
  ...
  async searchByFileName(fileName) {
    ...
    name = name.replace(/\[.*?\]/g, '');
    name = name.replace(/\(.*?\)/g, '');
    name = name.replace(/(epub|mobi|pdf|txt|zip|rar|轻小说|完结|全本|第.*卷)/gi, '');
    name = name.trim();
