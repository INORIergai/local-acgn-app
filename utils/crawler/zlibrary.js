...
class ZLibraryCrawler {
  ...
  async searchByFileName(fileName) {
    ...
    // ★ 相似度闸门（round26续）：宁可不配图，绝不写错封面。
    //   原实现 best 初值 = results[0]，且没有最低分门槛 ⇒ 只要 search 返回了任何结果，
    //   哪怕标题毫不相干，也会把 results[0] 的封面写进库（配错图比没图更糟）。
    //   现统一走 title-match 的 pickBest（阈值 0.6），不达标直接放弃。
    const { pickBest } = require('./title-match');
    const best = pickBest(results, name, 0.6);
    if (!best) {
      console.log(`[Z-Library] 「${name}」${results.length} 条候选均未达相似度阈值 0.6，放弃（宁可不配图）`);
      return null;
    }
