...
class QuarkCrawler {
  ...
  async _get(path, params = {}) {
    try {
      ...
      try {
        ...
        this._throwIfApiError(json, response.status, text);
        return json;
      } catch (e) {
        if (e.isApiError || e.message.includes('API错误') || e.message.includes('认证失败')) {
          throw e;
        }
  ...
  /**
   * ★ round26 续 #7：把「API 业务错误」和「HTTP 层错误」都变成**保留原文**的异常。
   *
   * 老实现有两个毛病，都会把真话吃掉：
   *  ① 业务错误（如 code 23018 `download file size limit`）被 catch 误判为「响应不是有效的JSON格式」，
   *     真实原因只留在括号里那 120 字里 —— 排查时极容易被带偏；
   *  ② HTTP 4xx 但响应体是合法 JSON（如 `{"status":404,"error":"Not Found","path":"/1/clouddrive/capacity"}`）
   *     因为**没有 code 字段**而被当成正常返回 ⇒ `data` 为 undefined ⇒ 静默失败
   *     （`/quark/storage` 就是被这个坑掉了一直报「获取存储空间信息失败」）。
   */
  _throwIfApiError(json, httpStatus, rawText) {
    ...
    const st = (json && json.status) || httpStatus;
    if (st >= 400) {
      const err = new Error(`夸克接口错误 HTTP ${st}: ${(json && (json.message || json.error)) || String(rawText).slice(0, 120)}`);
      err.isApiError = true;
      err.apiCode = st;
      throw err;
    }
   ...
   * 获取存储空间信息
   *
   * ★ round26 续 #7（顺手修）：老实现打 `GET /capacity`，该接口**已下线** ——
   *   实测返回 HTTP 404 `{"status":404,"error":"Not Found","path":"/1/clouddrive/capacity"}`，
   *   而 `_get` 只检查 `json.code`，这个 404 体没有 code 字段 ⇒ 不抛错、`result.data` 为 undefined
   *   ⇒ 永远返回 null ⇒ 路由永远回「获取存储空间信息失败」。
   *   容量数据现在在 `GET /member` 里：`data.use_capacity` / `data.total_capacity`。
   */
  async getStorageInfo() {
    try {
      const result = await this._get('/member');
      const d = result && result.data;
      if (!d || d.total_capacity == null) {
        console.log('[夸克网盘] 获取存储空间信息失败: /member 未返回 total_capacity');
        return null;
      }
      const total = Number(d.total_capacity) || 0;
      const used = Number(d.use_capacity) || 0;
      return {
        total,
        used,
        free: Math.max(0, total - used),
        // 保留原字段名，方便前端直接显示 / 后续扩展
        total_capacity: total,
        use_capacity: used,
        member_type: d.member_type || '',
        exp_at: d.exp_at || d.exp_svip_exp_at || null
      };
    } catch (e) {
