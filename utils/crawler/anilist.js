...
query ($s: String) {
  Page(page: 1, perPage: 10) {
    media(search: $s, type: ANIME, isAdult: true, sort: [SEARCH_MATCH]) {
      id
 ...
 */
async function gql(keyword) {
  const body = JSON.stringify({ query: GRAPHQL, variables: { s: keyword } });
 ...
 */
async function searchAniListAll(keyword) {
  const kw = String(keyword || '').trim();
  if (!kw) return [];
  const variants = keywordVariants(kw).slice(0, 3);
  for (const v of variants) {
    try {
      const list = await gql(v);
 ...
 */
async function searchAniListByName(keyword) {
  const kw = String(keyword || '').trim();
  if (!kw) return null;
  const cands = await searchAniListAll(kw);
  if (cands.length === 0) {
  ...
  if (!best) {
    console.log(`[AniList] 有 ${cands.length} 条候选但相关度不足，放弃（避免配错封面）`);
    return null;
  }
