...
// ========== 影视库/动漫库（round34 新四库） ==========
const { searchFilmByName } = require('./film-scraper');
const { searchCartoonByName, cleanCartoonName } = require('./bangumi-scraper');
...
function extractAvId(filename) {
  ...
  // 【2026-09-25 取证补齐】以下前缀此前缺席白名单 ⇒ extractAvId 返 confidence='low'
  // ⇒ searchMovie 在联网之前就直接 return null（见下方「低置信度番号不进行网络刮削」）。
  // 实测用户库 72 条「标题只剩番号」的影片里 69 条死于此：MIMK 13 / URE 8 / OFJE 5 /
  // WAAA 4 / JUR 4 / ABF 3 / SNIS 2 / MIDA 2 / OFES 2 / RHTS 2；TPRO、TYSF 各 1
  // （这两条置信度本就为高，属源站未收录，加前缀无副作用）。
  // 误配风险由「结果番号须与搜索番号完全一致」把关，白名单只需保证前缀形似番号。
  validPrefixes.push(
    'ABF', 'JUR', 'MIDA', 'MIMK', 'OFES', 'OFJE', 'RHTS',
    'SNIS', 'TPRO', 'TYSF', 'URE', 'WAAA'
  );

  // 【2026-09-25 round27 二次补齐】「一键重刮 41 条全失败」里 18 条死于此：
  // APAK / DASS / CAWD / DVMM / CJOD / ROYD / FNS / PPT / PPX / VGD / GVG / HOMA /
  // FOCS / MUDR —— 全部是真实厂牌，只因白名单没写就判 confidence='low'。
  validPrefixes.push(
    'APAK', 'CAWD', 'CJOD', 'DASS', 'DVMM', 'FNS', 'FOCS', 'GVG',
    'HOMA', 'MUDR', 'PPX', 'PPT', 'ROYD', 'VGD'
  );
...
/**
 * 文件名是否**以**标准番号开头（`APAK-284(1).mp4` / `DASS-683 何かのタイトル.mp4`）。
 * 这类串里番号的位置唯一、几乎没有误判空间。
 */
const LEADING_AVID_RE = /^[\[\(【\s]*([A-Za-z]{2,8})\s*-?\s*(\d{2,6})(?![0-9])/;
function hasLeadingAvid(filename) {
  const name = String(filename || '').replace(/\.[^.]+$/, '');
  return LEADING_AVID_RE.test(name);
}
...
async function searchMovie(cleanName, fileName, type, filePath) {
  ...
  // ========== 影视库（round34）：TMDB 主源 + 豆瓣备用 ==========
  if (type === 'film') {
    const filmResult = await searchFilmByName(cleanName, fileName);
    // film-scraper 直接返回归一化结构，不再过 normalizeResult
    return filmResult;
  }

  // ========== 动漫库（round34，普通向）：AniList(isAdult:false) 主源 + Bangumi 备源 ==========
  if (type === 'cartoon') {
    // 关键词：cleanCartoonName 会智能去掉字幕组组名（组名会污染 AniList/Bangumi 全文检索），
    // 空了再退到现有 animeSearchKey（取父文件夹名）逻辑。
    const cartoonKey = cleanCartoonName(fileName) || animeSearchKey(fileName, filePath) || cleanName;
    console.log(`[动漫搜索]清洗后关键词:${cartoonKey || '(空，放弃)'}`);
    if (!cartoonKey) return null;

    const { simScore, norm } = require('./crawler/title-match');
    const qn = norm(cartoonKey);
    const gate = (r, srcName) => {
      if (!r || !r.cover) return null;
      const s = simScore(r.title || '', qn);
      if (s < 0.5) {
    ...
    // 第一源：AniList 普通向（isAdult:false —— 与里番库的关键区别）
    if (config.sources?.anilist?.enabled !== false) {
      const alResult = gate(await searchAniListByName(cartoonKey, { isAdult: false }), 'AniList普通向');
      if (alResult) {
        console.log('[AniList普通向]命中:', alResult.title);
        return normalizeResult(alResult, 'anilist', null);
      }
      console.log('[AniList普通向]未检索到结果，换 Bangumi');
    }

    // 第二源：Bangumi（bgm.tv 官方 API，中文条目名，免 key）
    if (config.sources?.bangumi?.enabled !== false) {
      const bgmResult = await searchCartoonByName(cartoonKey, fileName);
      if (bgmResult) {
        return bgmResult;   // 已是归一化结构
      }
    }
    return null;
  }
  ...
  /* ★ 2026-09-25 round27：低置信度闸门放宽（原来一律 return null）
   *
   * 原逻辑：不在硬编码白名单里 ⇒ confidence='low' ⇒ **联网之前**直接放弃。
   * 实测代价：用户库 APAK/DASS/CAWD/DVMM/CJOD/ROYD/FNS/PPT/PPX/VGD/GVG/HOMA/FOCS/MUDR
   * 等真实厂牌全缺席，18 条片子永远「重新刮削失败」，用户体感是「点了 100 次一次都没成功」。
   * 白名单打补丁治标不治本（本文件已经补过两轮）。
   *
   * 误配风险本来也不靠这道闸门兜：下方有「结果番号必须与搜索番号完全一致」的硬校验，
   * 且各家 search 都是**按番号精确检索**。所以最坏结果只是「没搜到 = 维持现状」。
   * 折中：只放行「文件名以标准番号开头」的情况（位置唯一、误判空间小）；
   * 番号夹在中间/末尾的（`xxx_12345_HD.mp4` 这类）维持跳过。
   */
  if (avid && avidConfidence === 'low' && !hasLeadingAvid(fileName)) {
    console.log(`[刮削] 番号 ${avid} 置信度低且不在开头，跳过网络刮削`);
    return null;
  }
  if (avid && avidConfidence === 'low') {
    console.log(`[刮削] 番号 ${avid} 置信度低但位于文件名开头，放行尝试（结果仍按番号精确校验）`);
  }
...
async function enrichMissingFields(result, avid, fromSource) {
  try {
    ...
    // 【2026-09-25 round27】标题同样算「缺」：首个命中源给的标题常常只是番号。
    // 原设计是「绝不动 title」（防跨源图文错配），但这里所有候选源用的都是
    // **同一个番号**（进入本函数前已校验番号完全一致），所以采纳其它源的标题是安全的。
    // 实测 17 条「重刮成功但标题仍是番号」全部卡在这里。
    const titleMissing = () => !result.title || titleIsCodeOnly(result.title);
    if (need.length === 0 && !needGenres && !titleMissing()) return;
    ...
    for (const src of candidates) {
      ...
      if (stillNeed.length === 0 && (!needGenres || result.genres?.length) && !titleMissing()) break;
      asked++;
      ...
      try {
        ...
        // 标题：仅当当前是空/纯番号时采纳其它源的真实标题；已有真标题则绝不动
        if (titleMissing() && extra.title && !titleIsCodeOnly(extra.title)) {
...
/**
 * 判断一个标题是否「只有番号」——形如 `[OFJE-334]` / `OFJE 334` / `OFJE-334`。
 * 扫描侧用它区分「真数据」与「失败兜底产物」：刮削失败时导出的 NFO，标题就是文件名
 * 清洗结果（即番号本身）；若把这种记录当成「本地数据完整」，它会被永久锁在本地命中
 * 分支，连「一键重刮失败项」都绕不过（内部走的是同一条判断）。
 * 与 cleanMovieName 的分工：这里只判定、不改造字符串。
 */
const CODE_ONLY_TITLE_RE = /^[\[\]【】()（）\s]*[A-Za-z]{1,8}[\s\-_]?\d{2,6}[\]】)）\s]*$/;
function titleIsCodeOnly(title) {
  const t = String(title == null ? '' : title).trim();
  return !!t && CODE_ONLY_TITLE_RE.test(t);
}
...
module.exports = {
  ...
  detectTypeByPath,
  extractAvId,
  hasLeadingAvid,   // round27：低置信度闸门的放行判据，导出便于测试
  titleIsCodeOnly,
  searchMovie,
  downloadPoster,
  searchAllPosters,
