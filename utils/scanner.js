...
async function processSingleFile(filePath, type = 'jav') {
  ...
  // 4. 本地数据不全时，才尝试网络刮削补全
  let movieInfo = null;
  const hasFullData = nfoData && nfoData.title && localPoster;
  ...
  const finalTitle = nfoData?.title || movieInfo?.title || cleanName;
  const finalOriginalTitle = nfoData?.originalTitle || movieInfo?.original_title || fileName;
...
async function startRescrapeFailed() {
    ...
    for (const f of failures) {
        ...
        try {
            ...
            // 判定成功：刮到了真实标题（不再等于清理后的文件名）或标题发生变化
            const scraped = after && after.title && after.title !== after.cleanName;
            if (scraped) {
                clearScrapeFailure.run(f.filePath);
                rescrapeStatus.success++;
