...
// 女优列表
// round34 切片器：?type=jav|anime|film|cartoon 时按库过滤（JOIN movies 统计该库内的作品数）
router.get('/', (req, res) => {
    try {
        ...
        if (type && type !== 'all') {
            list = db.prepare(`
                SELECT a.*, COUNT(m.id) AS movieCount
                FROM actresses a
                JOIN movie_actress ma ON ma.actressId = a.id
                JOIN movies m ON m.id = ma.movieId
                WHERE m.type = ?
                GROUP BY a.id
                ORDER BY movieCount DESC, a.name ASC
            `).all(type);
        } else {
            list = getAllActresses.all();
        }
