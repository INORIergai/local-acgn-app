...
router.get('/annual', (req, res) => {
    try {
        ...
        // round34 批次B：type 分册（jav/anime/film/cartoon/comic/novel），不传 = 全部
        const type = (req.query.type || '').toString();
        const validTypes = ['jav', 'anime', 'film', 'cartoon', 'comic', 'novel'];
        const typeOn = validTypes.includes(type);
        const mTypeCond = typeOn ? ' AND m.type = ? ' : '';
        const mTypeParam = typeOn ? [type] : [];
        ...
        const playsByMonth = db.prepare(`
            ...
            WHERE playTime >= ? AND playTime < ? ${mTypeCond}
            GROUP BY month ORDER BY month
        `).all(yearStart, yearEnd, ...mTypeParam);
        ...
        const addedByMonth = db.prepare(`
            SELECT strftime('%m', datetime(addedTime / 1000, 'unixepoch', 'localtime')) as month,
                   COUNT(*) as added
            FROM movies
            WHERE addedTime >= ? AND addedTime < ? ${typeOn ? ' AND type = ? ' : ''}
            GROUP BY month ORDER BY month
        `).all(yearStart, yearEnd, ...mTypeParam);
        ...
        const watchSummary = db.prepare(`
            ...
            FROM watch_history wh
            JOIN movies m ON m.id = wh.movieId
            WHERE wh.playTime >= ? AND wh.playTime < ? ${mTypeCond}
        `).get(yearStart, yearEnd, ...mTypeParam);
        ...
        // 年内新增按库分布（type 分册时无意义，仍然给出全库分布作参照）
