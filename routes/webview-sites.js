...
const SITES = {
  ...
  // koobone：与 moxmoe(Kmoe) 同属 KOOB.ONE 站群，走的是网页版入口 /web.htm
  //   （页面标题 KOOB.ONE，登录框写「使用 Kmoe 帳號登錄」—— 同一套账号体系，不是另一家站）。
  //   实测直连 200，**没有任何 X-Frame-Options、也没有 CSP frame-ancestors**
  //   ⇒ 可内嵌，直接 iframe 即可，不需要整站反代（同 moxmoe/komiic/manhuagui 那批）。
  //   frameable:true 是实测结论，用来让 /api/webview/site **跳过那次多余的探测**：
  //     不标的话，本机代理没开时探测会失败，pane 上白挂一条「探测失败（网络抖动）」提示，
  //     而站点本身其实好好的。
  //   addrs 只放一个地址（带路径）—— 单地址站 resolveSite 会直接返回、不做逐级探测，
  //     多放一个同域镜像只会平白多等一轮超时。不加 listPath：站内是登录后的网盘式浏览，
  //     没有可抓的公开列表页。
  koobone: {
    name: 'Koobone',
    addrs: ['https://koobone.com/web.htm'],
    proxy: false,
    direct: true,
    frameable: true
  },
