...
router.post('/update/:id', (req, res) => {
    try {
        ...
        db.prepare(`
            ...
            WHERE id = ?
        `).run(title, overview, releaseDate, producer, serial, director, req.params.id);
...
router.post('/auto-cover/batch', async (req, res) => {
    ...
    autoCoverBatch(db, {
        ...
        console.log(`[补封面] 完成 成功${r.ok} 失败${r.fail}`);
    }).catch(e => {
        _coverJob.running = false;
...
router.post('/:id/rename', async (req, res) => {
    try {
      ...
      WHERE id = ?
    `).run(newPath, realNewFileName, newCleanName, req.params.id);

        // 标题本来就是从文件名推出来的（没刮到标题）时跟着一起改，
        // 否则改完名字界面上还是旧名字，看起来像没生效。
        if (!movie.title || movie.title === oldStem || movie.title === movie.cleanName) {
            db.prepare('UPDATE movies SET title = ? WHERE id = ?').run(newName, req.params.id);
        }

        return res.json({ code: 0, msg: '重命名成功', data: { fileName: realNewFileName } });
    } catch (e) {
        return res.json({ code: -1, msg: e.message });
    }
});
