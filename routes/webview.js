...
const BOARD_OF = {
  ...
  cycanime: 'anime',
  moxmoe: 'comic', komiic: 'comic', manhuagui: 'comic', wenku8: 'comic', koobone: 'comic'
};
...
router.get('/sites', (req, res) => {
  res.json({
    ...
    data: Object.entries(SITES).map(([key, s]) => ({
      ...
      // 搜索路径模板（round34 批次B：榜单「去观看」带词打开要用）
      searchPath: s.searchPath || null,
...
router.get('/site', async (req, res) => {
  ...
  // 站点表里已实测声明「可内嵌」（frameable:true）的，直接给结论，不再发一次探测请求：
  //   ① 省掉一次整页抓取（多则十几秒）；
  //   ② 避免「本机代理没开 → 探测失败 → 明明能嵌却给用户挂一条探测失败提示」这种误报。
  const probe = site.frameable === true
    ? { frameable: true, unknown: false, status: 0, xfo: '', reason: '' }
