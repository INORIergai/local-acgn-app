...
const READER_KEEP_TAGS = new Set([
    'p', 'div', 'span', 'br', 'hr', 'img', 'svg', 'image', 'g',
    'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
...
function htmlForReader(html, resolve) {
    ...
    // 2) 先处理 <svg><image xlink:href="..."/>（Kadokawa 的整页图就是这种）
    html = html.replace(/<image\b([^>]*?)\/?>/gi, (m, attrs) => {
        ...
        return `<img class="novel-illus novel-pageimg" src="${escapeAttr(src)}" alt="" loading="lazy">`;
    });
    ...
    html = html.replace(/<img\b([^>]*?)\/?>/gi, (m, attrs) => {
        ...
        return `<img class="novel-illus${clsM ? ' ' + escapeAttr(clsM[1]) : ''}" src="${escapeAttr(src)}" alt="${escapeAttr(altM ? altM[1] : '')}" loading="lazy">`;
    });
