...
// 标签列表（带影片数统计）
// round34 切片器：?type=jav|anime|film|cartoon 时按库过滤并重算计数
router.get('/', (req, res) => {
    try {
        ...
        if (type && type !== 'all') {
            list = db.prepare(`
                SELECT t.id, t.name, t.category, COUNT(m.id) AS movieCount
                FROM tags t
                JOIN movie_tag mt ON mt.tagId = t.id
                JOIN movies m ON m.id = mt.movieId
                WHERE m.type = ?
                GROUP BY t.id
                ORDER BY movieCount DESC, t.name ASC
            `).all(type);
        } else {
            list = getAllTags.all();
        }
