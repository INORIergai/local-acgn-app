...
/* ============================================================================
   ★ round26 续 #7：夸克「显示已登录、直链却被拒」的可诊断化
   ----------------------------------------------------------------------------
   真因（2026-09-25 在容器内实测定位，非猜测）：
     夸克直链是阿里 OSS 签名地址，URL 上带 callback 参数。CDN 收到请求后会
     回呼 drive-auth.quark.cn/outer/oss/checkplay，并把本次请求的
     cookie / referer / ip 转发过去校验。会话过期时回呼直接拒绝，返回：

       <Code>RequestDeniedByCallback</Code>
       <Message>Callback deny this request reason: require login [auth expired]</Message>

     对照实验（同一根直链、只改一个头）：
       带 Cookie  → require login [auth **expired**]
       不带 Cookie → require login [auth **not found**]

     致命之处：/file/sort、/file/info、/file/download 这些 API 用同一个过期 cookie
     仍然返回 200 —— 于是 checkLogin() 报「已登录」，用户点开 PDF 只看到
     「夸克 CDN 返回 HTTP 403」，完全想不到要去重新扫码登录。
     这里把 XML 判词翻译成人话，并记住该状态供前端提示。
   ============================================================================ */
let quarkAuthExpiredAt = 0;   // 最近一次确认「会话过期」的时间戳；0 = 未发现
let quarkAuthLastDetail = ''; // 原始判词，便于排查

// 读 CDN 的错误正文并翻译；读不出可辨识的判词则返回 null（交回原逻辑）
async function describeQuarkCdnDeny(resp) {
    try {
        const text = (await resp.text()).slice(0, 2000);
        const raw = text.replace(/\s+/g, ' ').trim().slice(0, 300);
        if (/RequestDeniedByCallback/i.test(text)) {
            quarkAuthExpiredAt = Date.now();
            quarkAuthLastDetail = raw;
            if (/auth\s*expired/i.test(text)) {
                return { msg: '夸克登录已过期，请在「夸克网盘」里重新扫码登录后再打开', raw };
            }
            if (/auth\s*not\s*found/i.test(text)) {
                return { msg: '夸克尚未登录（或 Cookie 缺失），请先扫码登录夸克网盘', raw };
            }
            return { msg: '夸克直链被拒绝（RequestDeniedByCallback），通常是登录状态失效，请重新扫码登录', raw };
        }
        if (resp.status === 403) {
            quarkAuthExpiredAt = Date.now();
            quarkAuthLastDetail = raw;
            return { msg: '夸克直链返回 403（多半是登录状态失效），请重新扫码登录后再试', raw };
        }
    } catch (_) { }
    return null;
}
...
router.post('/quark/set-cookie', (req, res) => {
    try {
        ...
        // ★ 换了新 cookie，之前记下的「会话过期」标记必须清掉，否则界面一直显示过期
        quarkAuthExpiredAt = 0;
        quarkAuthLastDetail = '';
...
// 检查夸克网盘登录状态
// ★ round26 续 #7：这里必须区分「API 认得出 cookie」和「CDN 还认这个会话」。
//   夸克 API（/file/sort 等）对过期 cookie 依然返回 200，所以只靠 loggedIn
//   会稳定误报「已登录」，而真正的下载/在线阅读全部 403。
//   sessionValid = 最近一次实际取直链时是否被 CDN 以「登录过期」拒绝过。
router.get('/quark/check-login', async (req, res) => {
    try {
        const crawler = getQuarkCrawler();
        const loggedIn = await crawler.checkLogin();
        const authExpired = !!quarkAuthExpiredAt;

        res.json({
            code: 0,
            data: {
                loggedIn,
                sessionValid: loggedIn && !authExpired,
                authExpiredAt: quarkAuthExpiredAt || null,
                reason: authExpired
                    ? '夸克登录已过期：文件列表还能读，但下载/在线阅读会被拒绝，请重新扫码登录'
                    : '',
                detail: authExpired ? quarkAuthLastDetail : ''
            }
        });
    } catch (e) {
        res.json({ code: -1, msg: e.message });
...
/* ★ round26 续 #7：夸克「登录态」的主动验证
   ----------------------------------------------------------------------------
   为什么需要它：`loggedIn` 只证明「API 认这个 cookie」，而会话过期时 API 依然返回 200，
   所以 `loggedIn=true` 并**不能**说明「下载/在线阅读能用」。
   重新扫码登录后，如果不真去取一次流，用户会看到「登录成功」然后再次撞 403。
   这里取网盘里**最小的一个文件**真取 1 字节，用结果回答「到底能不能读」。
   查询：GET /api/cloud/quark/health
   ============================================================================ */
router.get('/quark/health', async (req, res) => {
    try {
        const crawler = getQuarkCrawler();
        const loggedIn = await crawler.checkLogin();
        if (!loggedIn) {
            return res.json({ code: 0, data: { loggedIn: false, sessionValid: false, msg: '夸克未登录或 Cookie 已失效' } });
        }

        /* 候选文件：根目录前 3 页里最小的若干个。
           ★ 必须多试几个，且**只在拿到确诊证据时**才判 false —— 实测两种「不是会话问题」的干扰：
             · 图片类直链会被 Tengine 防盗链挡下（412，带不带 Cookie 完全一样，与登录态无关）；
             · 超过约 50MB 的文件拿不到直链（code 23018 `download file size limit`）。
           把这两种当成「会话过期」就是假阴性，会把用户引到错误的方向去重新登录。 */
        const cands = [];
        for (let page = 1; page <= 3 && cands.length < 30; page++) {
            let lr;
            try { lr = await crawler.getFileList('0', page, 100); } catch (_) { break; }
            for (const f of ((lr && lr.list) || [])) {
                if (f.file_type === 0 || f.dir === true) continue;
                if ((f.size || 0) <= 0) continue;
                cands.push(f);
            }
        }
        cands.sort((a, b) => (a.size || 0) - (b.size || 0));

        const tried = [];
        for (const f of cands.slice(0, 5)) {
            const mb = +((f.size || 0) / 1048576).toFixed(2);
            const url = await crawler.getDownloadUrl(f.fid);
            if (!url) {
                tried.push({ name: f.file_name, sizeMB: mb, stage: '直链', why: crawler.lastDownloadError || '拿不到直链' });
                continue;
            }
            const headers = quarkDownloadHeaders();
            headers.Range = 'bytes=0-0';
            const resp = await fetch(url, quarkFetchOptions({ headers }));
            if (resp.ok || resp.status === 206) {
                quarkAuthExpiredAt = 0;             // 取流成功 ⇒ 清掉「过期」标记
                quarkAuthLastDetail = '';
                return res.json({
                    code: 0, data: {
                        loggedIn: true, sessionValid: true,
                        msg: '取流验证通过，下载 / 在线阅读可用',
                        tested: { name: f.file_name, sizeMB: mb, HTTP: resp.status }
                    }
                });
            }
            const deny = await describeQuarkCdnDeny(resp);   // 403 + RequestDeniedByCallback 才是确诊
            if (deny) {
                return res.json({
                    code: 0, data: {
                        loggedIn: true, sessionValid: false,
                        msg: deny.msg, detail: deny.raw,
                        tested: { name: f.file_name, sizeMB: mb, HTTP: resp.status }
                    }
                });
            }
            tried.push({ name: f.file_name, sizeMB: mb, stage: '取流', HTTP: resp.status });
        }

        // 没拿到确诊证据 ⇒ 如实说「无法判定」，不猜
        return res.json({
            code: 0, data: {
                loggedIn: true, sessionValid: null,
                msg: cands.length
                    ? '自动探测无法判定（候选文件都被防盗链或大小限制挡住了），请手动打开一个 PDF / EPUB 确认'
                    : '网盘根目录没有可测文件，无法自动验证（请手动打开一个文件试试）',
                tried
            }
        });
    } catch (e) {
...
router.get('/quark/stream', async (req, res) => {
    try {
        ...
        if (!resp.ok) {
            // 先翻译 CDN 判词：登录过期的 403 要给出可执行的动作，而不是一句 HTTP 403
            const deny = await describeQuarkCdnDeny(resp);
            if (deny) {
                return res.status(401).json({
                    code: -1, code2: 'QUARK_AUTH_EXPIRED',
                    msg: deny.msg, detail: deny.raw
                });
            }
...
router.get('/quark/read/pdf', async (req, res) => {
    try {
        ...
        if (!resp.ok) {
            const deny = await describeQuarkCdnDeny(resp);
            return res.status(deny ? 401 : resp.status).json({
                code: -1,
                code2: deny ? 'QUARK_AUTH_EXPIRED' : undefined,
                msg: deny ? deny.msg : `下载失败 HTTP ${resp.status}`
            });
        }
...
async function downloadBuffer(downloadUrl) {
    ...
    if (!resp.ok) {
        // ★ 同样先翻译判词：会话过期时要报「请重新登录」，别只报 HTTP 403
        const deny = await describeQuarkCdnDeny(resp);
        throw new Error(deny ? deny.msg : `下载失败 HTTP ${resp.status}`);
    }
